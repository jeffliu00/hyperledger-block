...
export class SmartContractUtil {
    ...
    public static async submitTransaction(contractName: string, functionName: string, args: string[], gateway): Promise<Buffer> {
        ...
        if (contractName !== '') {
            contract = await network.getContract('block', contractName);
        } else {
            contract = await network.getContract('block');
        }
    ...
    public static async evaluateTransaction(contractName: string, functionName: string, args: string[], gateway): Promise<Buffer> {
